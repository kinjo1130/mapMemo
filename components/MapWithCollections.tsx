...
const MapWithCollections: React.FC<MapWithCollectionsProps> = ({ userId }) => {
  ...
  return (
    <div className="relative h-full w-full">
      ...
      {/* フィルターUIを地図の上に絶対配置 - ドロップダウン形式 */}
      <div className="absolute top-2 left-2 right-2 z-10">
        {/* トグルボタン */}
        <button
          onClick={() => setIsFilterOpen(!isFilterOpen)}
          className="w-full bg-white shadow-lg rounded-lg px-4 py-2 flex items-center justify-between hover:bg-gray-50 transition-colors"
        >
          <div className="flex items-center gap-2">
            <Search size={18} className="text-gray-600" />
            <span className="text-sm font-medium text-gray-700">フィルター・検索</span>
          </div>
          <ChevronDown
            size={20}
            className={`text-gray-600 transition-transform ${isFilterOpen ? 'rotate-180' : ''}`}
          />
        </button>

        {/* ドロップダウンコンテンツ */}
        {isFilterOpen && (
          <div className="mt-2 bg-white shadow-lg rounded-lg p-3 flex flex-col space-y-2">
            {/* 検索ボックス */}
